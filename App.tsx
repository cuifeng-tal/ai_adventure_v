...
const App: React.FC = () => {
  ...
  // 预生成的提示语音数据
  const preGeneratedErrorAudios = useRef<string[]>([]);
  const preGeneratedSuccessAudios = useRef<string[]>([]);
  const isPreGenerating = useRef<boolean>(false);

  const errorMessagesRef = useRef([
    "差一点点！勇敢的探险家，再试一次！",
    "哎呀，似乎还是不对。仔细想想题目给的线索。",
  ]);

  const successMessagesRef = useRef([
    "太棒了！你答对了！",
    "厉害！你的思维真敏捷！",
  ]);
  ...
  // 预生成错误和成功提示语音
  useEffect(() => {
    // 如果已经在生成中或已经生成完成，直接返回
    if (
      isPreGenerating.current ||
      (preGeneratedErrorAudios.current.length > 0 &&
        preGeneratedSuccessAudios.current.length > 0)
    ) {
      console.log("语音已预生成或正在生成中，跳过重复请求");
      return;
    }

    const preGenerateAudios = async () => {
      isPreGenerating.current = true;
      console.log("开始批量预生成提示语音...");

      // 并行生成所有错误和成功提示语音
      const allMessages = [
        ...errorMessagesRef.current.map((msg) => ({ msg, type: "error" })),
        ...successMessagesRef.current.map((msg) => ({ msg, type: "success" })),
      ];

      const results = await Promise.allSettled(
        allMessages.map(async ({ msg, type }) => {
          const audioData = await generateSpeech(msg);
          if (audioData) {
            console.log(
              `成功生成${type === "error" ? "错误" : "成功"}语音: ${msg}`
            );
            return { audioData, type };
          }
          throw new Error(`生成语音失败: ${msg}`);
        })
      );

      // 分类存储成功的结果
      const errorAudios: string[] = [];
      const successAudios: string[] = [];

      results.forEach((result, index) => {
        if (result.status === "fulfilled" && result.value) {
          if (result.value.type === "error") {
            errorAudios.push(result.value.audioData);
          } else {
            successAudios.push(result.value.audioData);
          }
        } else if (result.status === "rejected") {
          console.error(
            `语音生成失败 [${allMessages[index].msg}]:`,
            result.reason
          );
        }
      });

      preGeneratedErrorAudios.current = errorAudios;
      preGeneratedSuccessAudios.current = successAudios;
      isPreGenerating.current = false;

      console.log(
        `批量预生成完成，共 ${errorAudios.length} 条错误语音，${successAudios.length} 条成功语音`
      );
    };
  ...
  const playSpeech = async (text: string, preGeneratedAudio?: string) => {
    ...
    // 优先使用预生成的语音数据，如果没有则实时生成
    const audioData = preGeneratedAudio || (await generateSpeech(text));
  ...
  // 播放预生成的错误语音（非阻塞）
  const playPreGeneratedErrorAudio = async () => {
    // 如果正在播放，直接返回
    if (context.isAudioPlaying) {
      return;
    }
    ...
    // 如果没有预生成的语音，跳过
    if (preGeneratedErrorAudios.current.length === 0) {
      console.log("尚无预生成错误语音，跳过播放");
      return;
    }

    // 随机选择一条语音
    const randomIndex = Math.floor(
      Math.random() * preGeneratedErrorAudios.current.length
    );
    const audioData = preGeneratedErrorAudios.current[randomIndex];
  ...
  // 播放预生成的成功语音（非阻塞）
  const playPreGeneratedSuccessAudio = async () => {
    // 如果正在播放，直接返回
    if (context.isAudioPlaying) {
      return;
    }
    ...
    // 如果没有预生成的语音，跳过
    if (preGeneratedSuccessAudios.current.length === 0) {
      console.log("尚无预生成成功语音，跳过播放");
      return;
    }

    // 随机选择一条语音
    const randomIndex = Math.floor(
      Math.random() * preGeneratedSuccessAudios.current.length
    );
    const audioData = preGeneratedSuccessAudios.current[randomIndex];
  ...
  const handleStart = async (q: string) => {
    ...
    try {
      ...
      const [img, audioData] = await Promise.all([
        generateCartoonImage(levelContent.story),
        generateSpeech(levelContent.story),
      ]);
      ...
      // 使用预先生成的语音数据播放
      if (audioData) {
        playSpeech(levelContent.story, audioData);
      } else {
  ...
  const checkAnswerL1 = (ans: string) => {
    ...
    if (ans.trim().toLowerCase() === context.l1Answer.trim().toLowerCase()) {
      ...
      const randomMsgIndex = Math.floor(
        Math.random() * successMessagesRef.current.length
      );
      const successMsg = successMessagesRef.current[randomMsgIndex];

      setToastMsg(successMsg);
      playPreGeneratedSuccessAudio();
      setTimeout(() => setToastMsg(""), 3000);
      ...
      // 随机选择一条错误消息显示
      const randomMsgIndex = Math.floor(
        Math.random() * errorMessagesRef.current.length
      );
      const errorMsg = errorMessagesRef.current[randomMsgIndex];

      setErrorMsg(errorMsg);
      // 非阻塞播放预生成的语音
      playPreGeneratedErrorAudio();
  ...
  const startLevel2 = async (difficulty: Difficulty) => {
    ...
    try {
      ...
      // 并行生成图片和语音，减少等待时间
      const [img, audioData] = await Promise.all([
        generateCartoonImage(level2Content.story),
        generateSpeech(level2Content.story),
      ]);
      ...
      // 使用预先生成的语音数据播放
      if (audioData) {
        playSpeech(level2Content.story, audioData);
      } else {
    ...
    if (ans.trim().toLowerCase() === context.l2Answer.trim().toLowerCase()) {
      ...
      try {
        const abilityData = await generateGameContent(
          ...
          }，第一关尝试了${context.l1FailCount + 1}次成功，
  ...
  return (
    <GameLayout
      title={gameState === GameState.INITIAL ? "知识探险家" : "数学大冒险"}
    >
      {errorMsg && (
        ...
        <div className="text-center space-y-12 py-8 animate-in fade-in zoom-in duration-700">
          ...
          <div className="relative inline-block">
            <div className="w-64 h-64 bg-gradient-to-tr from-yellow-400 to-orange-300 rounded-full flex items-center justify-center border-[8px] border-black shadow-[15px_15px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden group">
              ...
              <span className="text-9xl group-hover:scale-110 transition-transform duration-500 relative z-10">
                🗺️
              </span>
            ...
            {/* Dynamic badges */}
            <div className="absolute -top-6 -right-6 w-20 h-20 bg-pink-400 rounded-2xl border-4 border-black flex items-center justify-center text-4xl shadow-lg rotate-12 animate-bounce">
              ✨
            </div>
            <div className="absolute -bottom-4 -left-8 w-16 h-16 bg-blue-400 rounded-full border-4 border-black flex items-center justify-center text-3xl shadow-lg -rotate-12 animate-float">
              🎒
            </div>
          </div>
          ...
          <div className="max-w-xl mx-auto space-y-8">
            <div className="relative group">
              ...
              <div className="relative">
                ...
                />
                <div
                  className="absolute right-6 top-1/2 -translate-y-1/2 text-4xl opacity-50 group-hover:scale-125 transition-transform"
                  style={{ cursor: "pointer" }}
                  onClick={(e) => handleStart(inputRef.current?.value || "")}
                >
                  🔑
                </div>
              </div>
          ...
          {loading && (
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-yellow-600 cartoon-font text-3xl animate-pulse">
                正在编织冒险故事的丝线...
              </div>
            </div>
          )}
      ...
      {gameState === GameState.LEVEL_1_FEEDBACK && (
        <div className="text-center space-y-12 py-16 animate-in zoom-in duration-500 bg-green-50 rounded-[3rem] border-[6px] border-black shadow-[10px_10px_0px_0px_rgba(34,197,94,1)]">
          ...
          {loading && (
            <div className="text-blue-600 cartoon-font text-2xl animate-pulse">
              🛠 正在构建新的试炼之地...
            </div>
          )}
        </div>
        ...
        <div className="space-y-10 py-6 animate-in slide-in-from-bottom duration-700">
          <div className="bg-orange-50 border-[8px] border-black p-10 rounded-[3rem] text-center relative overflow-hidden shadow-[12px_12px_0px_0px_rgba(249,115,22,1)]">
            ...
            <div className="flex justify-center -space-x-8 mb-10">
              <img
                src={context.l1Image}
                className="w-40 h-40 object-cover rounded-3xl border-[6px] border-black shadow-xl rotate-[-8deg] relative z-10 hover:z-30 transition-all hover:scale-110"
              />
              <img
                src={context.l2Image}
                className="w-40 h-40 object-cover rounded-3xl border-[6px] border-black shadow-xl rotate-[8deg] relative z-20 hover:z-30 transition-all hover:scale-110"
              />
            </div>

            <h2 className="text-6xl cartoon-font text-orange-600 mb-8 drop-shadow-md">
              传奇探险家！
            </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white border-[5px] border-black p-6 rounded-[2rem] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                    ...
                    <div className="h-8 w-full bg-gray-100 rounded-full border-[4px] border-black overflow-hidden p-1 shadow-inner">
                      <div
                        className="h-full bg-green-400 rounded-full border-r-[4px] border-black"
                        style={{ width: `${ability.mastery}%` }}
                      ></div>
                    </div>
                    <p className="text-4xl cartoon-font mt-4 text-green-600">
                      {ability.mastery}%
                    </p>
                  </div>
                  <div className="bg-white border-[5px] border-black p-6 rounded-[2rem] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                    ...
                    <div className="h-8 w-full bg-gray-100 rounded-full border-[4px] border-black overflow-hidden p-1 shadow-inner">
                      <div
                        className="h-full bg-blue-400 rounded-full border-r-[4px] border-black"
                        style={{ width: `${ability.logic}%` }}
                      ></div>
                    </div>
                    <p className="text-4xl cartoon-font mt-4 text-blue-600">
                      {ability.logic}%
                    </p>
                  </div>
                </div>
          ...
          <div className="flex justify-center gap-4 text-6xl opacity-50 animate-bounce">
            🏆 🎖️ 🏆
          </div>
        </div>
      )}
